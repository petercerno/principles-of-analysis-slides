...
\begin{frame}{Recall: Definition 2.18(h) --- Perfect Set}
  ...
  \vspace{0.5em}
  \begin{hlbox}
    \textbf{Equivalent form:} $E$ is perfect $\iff$ no isolated points, contains all limit points.
  \end{hlbox}
...
\begin{frame}{Proof --- Step 1: The Base Case}
  ...
  \vspace{0.4em}
  \hl{Since $\mathbf{x}_1 \in P$, we have $V_1 \cap P \neq \emptyset$.}
...
\begin{frame}{Visualizing Why $P$ is Perfect}
  ...
  \vspace{0.2em}
  \centerline{\hl{\scriptsize Choose $n$ large enough that $I_n \subset S$.}}
...
\begin{frame}{Summary}
  ...
  \note{
    Let us summarize the main ideas of this section. The headline theorem
    is Theorem 2.43: every nonempty perfect subset of "R" to the "k" is
    uncountable. So just two mild topological hypotheses, closed and no
    isolated points, force the set to have continuum many elements.
  }
\end{frame}
...
\begin{frame}{Summary}
  ...
  \note{
    The proof technique is worth remembering. We assumed the set was
    countable, listed its points, and built a sequence of shrinking
    neighborhoods, each one carefully chosen to kick out one listed point.
    The corollary to Theorem 2.36 then said the nested compact pieces had
    a common point, but our construction had excluded every listed point,
...
\begin{frame}{Summary}
  ...
  \note{
    A direct corollary is that every nondegenerate closed interval is
    perfect, since interior points are limit points from both sides and
    endpoints are limit points from inside. By Theorem 2.43, such an
    interval is uncountable, and therefore the entire real line is
    uncountable as well.
  }
\end{frame}
...
\begin{frame}{Summary}
  ...
  \note{
    We then constructed the Cantor set by repeatedly removing the open
    middle third of every surviving interval, starting from the unit
    interval. The resulting set "P" is compact and nonempty, contains
    no open segment, is perfect, and therefore by Theorem 2.43 is
    uncountable. The total length removed is 1, so "P" also has Lebesgue
    measure zero, as we will see in Chapter 11. It is simultaneously as
    big as the real line and as small as a single point in the
...
\begin{frame}{Summary}
  \begin{hlbox}
    ...
    \vspace{0.4em}
    \textbf{Coming up next:} Connected sets --- when is a set ``in one piece''?
  \end{hlbox}

  \note{
    In the next section we turn to connectedness, the precise notion of a
    set being in one piece. We will see how to formalize the intuitive
    idea that a set is not split into two separated parts, and we will
    relate connectedness in the real line to intervals. See you there.
  }
\end{frame}
}
