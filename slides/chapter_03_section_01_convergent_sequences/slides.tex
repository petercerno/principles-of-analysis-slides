...
\begin{frame}{Picture: What Convergence Looks Like}
  ...
  \begin{tikzpicture}[scale=0.95]
    ...
    \fill[white, opacity=0.3, rounded corners=3pt] (-3.8,-2.2) rectangle (4.2,2.2);
    ...
    \fill[defcolor] (-2.9,-1.5) circle (2pt) node[left=2pt]{\footnotesize $p_1$};
    ...
    \fill[defcolor] (0.6,0.6) circle (2pt);
    \fill[defcolor] (1.7,-0.2) circle (2pt);
    \fill[defcolor] (0.9,-0.3) circle (2pt);
    \fill[defcolor] (1.5,0.5) circle (2pt);
    \fill[defcolor] (1.1,0.1) circle (2pt);
    \fill[defcolor] (1.3,0.35) circle (2pt);
    % Arrow + label for "index N onward"
    \draw[thmcolor, thick, ->] (1.2,-1.9) -- (1.2,-0.75);
    \node[thmcolor, right=2pt] at (1.2,-1.6) {\footnotesize from index $N$};
  \end{tikzpicture}
  \end{center}

  \vspace{0.3em}
  \begin{center}
    \hl{No matter how small the $\varepsilon$-ball around $p$, \alert{eventually} every term lies inside.}
  \end{center}
...
\begin{frame}{Example 3.1 (a): $s_n = 1/n$}
  \begin{exblock}{Sequence: $s_n = \dfrac{1}{n}$}
    ...
    \]
    \begin{itemize}
      \item \makebox[2.5cm][l]{Converges:} $\lim_{n\to\infty} \frac{1}{n} = 0$
      \item \makebox[2.5cm][l]{Range:} infinite
      \item \makebox[2.5cm][l]{Bounded:} yes (all terms in $[0,1]$)
    \end{itemize}
  \end{exblock}

  \note{
    Example "a" is the reciprocal sequence: 1, one-half, one-third, and
    so on. Why does it converge to 0? Given any positive tolerance
    epsilon, we just need an index "N" large enough that 1 over "N"
    drops below epsilon, and the Archimedean property of the reals
    guarantees such an "N" exists. From that point on, every term sits
    within epsilon of 0. All the terms are distinct, so the range is
    infinite, and every term lies between 0 and 1, so the sequence is
    bounded. This will be our running model of a clean, well-behaved
    convergent sequence.
  }
\end{frame}
...
\begin{frame}{Example 3.1 (b): $s_n = n^2$}
  \begin{exblock}{Sequence: $s_n = n^2$}
    ...
    \]
    \begin{itemize}
      \item \makebox[2.5cm][l]{\alert{Diverges}:} grows without bound
      \item \makebox[2.5cm][l]{Range:} infinite
...
\begin{frame}{Example 3.1 (c): $s_n = 1 + (-1)^n/n$}
  \begin{exblock}{Sequence: $s_n = 1 + \dfrac{(-1)^n}{n}$}
    ...
    \]
    \begin{itemize}
      \item \makebox[2.5cm][l]{Converges:} $\displaystyle\lim_{n\to\infty} s_n = 1$
      \item \makebox[2.5cm][l]{Range:} infinite
      \item Bounded
    \end{itemize}
  \end{exblock}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{exblock}{Sequence: $s_n = i^n$}
        ...
        \]
        \begin{itemize}
          \item \makebox[2.5cm][l]{\alert{Diverges}:} cycles forever
          \item \makebox[2.5cm][l]{Range:} finite: $\{i, -1, -i, 1\}$
          \item \makebox[2.5cm][l]{Bounded:} on unit circle
        \end{itemize}
      \end{exblock}
    \end{column}
...
\begin{frame}{Example 3.1 (e): Constant Sequence $s_n = 1$}
  \begin{exblock}{Sequence: $s_n = 1$ for all $n$}
    ...
    \begin{itemize}
      \item \makebox[2.5cm][l]{Converges:} $\displaystyle\lim_{n\to\infty} s_n = 1$
      \item \makebox[2.5cm][l]{Range:} finite: $\{1\}$
      \item Bounded
    \end{itemize}
  \end{exblock}
...
\begin{frame}{Theorem 3.2: Four Properties of Convergent Sequences}
  \hl{Let $\{p_n\}$ be a sequence in a metric space $X$.}
  \begin{thmblock}{Theorem 3.2}
    \begin{enumerate}
      \item[(a)] $p_n \to p$ $\iff$ every $N_\varepsilon(p)$ contains $p_n$ for all but finitely many $n$.
    \end{enumerate}
  \end{thmblock}
...
\begin{frame}{Proof of 3.2(a): Forward Direction}
  \hl{\emph{(a):} $p_n \to p$ $\iff$ every $N_\varepsilon(p)$ contains $p_n$ for all but finitely many $n$.}

  \vspace{0.3em}
  \begin{proofblock}{$\Rightarrow$ direction}
    Suppose $p_n \to p$ and let $V = N_\varepsilon(p)$ be a neighborhood of $p$, $\varepsilon > 0$. \\
...
\begin{frame}{Proof of 3.2(a): Converse}
  ...
  \begin{proofblock}{$\Leftarrow$ direction}
    ...
    \vspace{0.3em}
    Fix $\varepsilon > 0$. Set $V = N_\varepsilon(p) = \{q \in X : d(p,q) < \varepsilon\}$ (the $\varepsilon$-ball).
...
\begin{frame}{Proof of 3.2(b): Uniqueness --- Setup}
  \hl{\emph{(b):} If $p_n \to p$ and $p_n \to p'$, then $p = p'$.}
...
\begin{frame}{Proof of 3.2(c): Convergent $\Rightarrow$ Bounded --- Setup}
  \hl{\emph{(c):} $\{p_n\}$ convergent $\;\Longrightarrow\;$ $\{p_n\}$ bounded.}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{tikzpicture}[scale=1.3]
        ...
        \fill[white, opacity=0.6] (0,0) circle (1.8);
        \draw[proofcolor, thick] (0,0) circle (1.8);
        \node[proofcolor] at (1.6,1.6) {\scriptsize radius $r$};
        % Inner unit ball
        \fill[proofcolor!15] (0,0) circle (0.9);
        \draw[proofcolor, dashed] (0,0) circle (0.9);
        \node[proofcolor] at (0.9,0.9) {\scriptsize radius $1$};
        % p
        \fill[mAccent] (0,0) circle (2pt) node[above right=-1pt]{\scriptsize $p$};
        % Initial segment (outside unit, inside r)
        \fill[defcolor] (-1.2,0.8) circle (1.5pt) node[left=-1pt]{\scriptsize $p_1$};
        \fill[defcolor] (1.0,-1.0) circle (1.5pt) node[right=-1pt]{\scriptsize $p_2$};
        \fill[defcolor] (-0.5,-1.4) circle (1.5pt) node[below=-1pt]{\scriptsize $p_3$};
        % Tail (inside unit)
...
\begin{frame}{Proof of 3.2(d): Limit Point $\Rightarrow$ Sequential Limit}
  \hl{\emph{(d):} If $p$ is a limit point of $E \subset X$, then $\exists\, \{p_n\} \subset E$ with $p_n \to p$.}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[scale=1.55]
        ...
        \fill[defcolor] (-1.0,-0.7) circle (1.5pt) node[left=-1pt]{\scriptsize $p_1$};
        % p_2 inside 1/2 (outside 1/3)
        \fill[defcolor] (0.45,-0.45) circle (1.5pt) node[right=-1pt]{\scriptsize $p_2$};
        % p_3 inside 1/3 (outside 1/4)
        \fill[defcolor] (-0.35,0.3) circle (1.3pt) node[left=-1pt]{\scriptsize $p_3$};
        % p_4 inside 1/4
        \fill[defcolor] (-0.2,-0.1) circle (1.2pt) node[below=-1pt]{\scriptsize $p_4$};
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{Theorem 3.3: Arithmetic of Limits}
  \hl{Suppose $s_n \to s$ and $t_n \to t$ in $\mathbb{C}$. Then:}
...
\begin{frame}{Proof of 3.3(a): Sum}
  ...
  \note{
    The proof uses what is sometimes called the split-epsilon trick.
    The idea is to give each summand its own budget. We want the total
    deviation to stay below epsilon, and the sum has two pieces, so we
    allot epsilon over 2 to each. The convergence of "s" sub "n" to
    "s" gives an index "N" sub 1 past which the first piece sits
    inside its budget, and the convergence of "t" sub "n" to "t" gives
    an index "N" sub 2 for the second piece. Beyond the larger of the
    two indices, both budgets hold at once, and the triangle
    inequality glues them together. The deviation of the sum is at
    most the sum of the two pieces, which is exactly epsilon. That is
    convergence of the sum, with the same epsilon that started the
    argument.
  }
\end{frame}
...
\begin{frame}{Note on 3.3(b): Scalar and Constant}
  \begin{remblock}{Constant addition: $\displaystyle\lim (c + s_n) = c + s$}
    The constant sequence $t_n := c$ converges to $c$ (every term equals the limit).
    Apply 3.3(a) to $s_n$ and $t_n$.
  \end{remblock}

  \vspace{0.3em}
  \begin{proofblock}{Scalar multiplication: $\displaystyle\lim c\, s_n = c\, s$}
    If $c = 0$, both sides equal $0$. Otherwise, given $\varepsilon > 0$, pick $N$ with
    \[
      n \geq N \;\Longrightarrow\; |s_n - s| < \frac{\varepsilon}{|c|}.
    \]
    Then $|c\, s_n - c\, s| = |c|\,|s_n - s| < \varepsilon$. \;$\blacksquare$
  \end{proofblock}

  \note{
    For part "b", the addition statement is essentially free. The
    constant sequence whose every term equals "c" converges to "c",
    because the distance from each term to the limit is always zero.
    Adding "c" to "s" sub "n" is then just a sum of two convergent
    sequences, which part "a" already handles. The scalar
    multiplication statement deserves a direct epsilon proof. If "c"
    equals zero, both sides are zero, so there is nothing to check.
    Otherwise, given any positive epsilon, the convergence of "s" sub
    "n" to "s" lets us pick an index "N" beyond which the distance
    from "s" sub "n" to "s" is less than epsilon divided by the
    absolute value of "c". Multiplying through by the absolute value
    of "c", the distance from "c" times "s" sub "n" to "c" times "s"
    is less than epsilon. So "c" times "s" sub "n" converges to "c"
    times "s".
  }
\end{frame}
...
\begin{frame}{Proof of 3.3(c): Product --- The Key Identity}
  ...
  \note{
    Part "c" is the one with a real trick, and the trick is the
    identity on the slide. It rewrites the difference between "s"
    sub "n" times "t" sub "n" and the target product "s" times "t"
    as a sum of three pieces. You can verify the identity by
    expanding the right side and watching the cross terms cancel; it
    is just rearrangement. The reason for choosing this particular
    rearrangement is that every piece on the right is built only out
    of the differences "s" sub "n" minus "s" and "t" sub "n" minus
    "t", which our convergence hypotheses let us drive to zero. The
    first piece is a product of two small quantities and will be
    quadratically small. The other two pieces are each a fixed
    constant, "s" or "t", times a small quantity, so they are
    linearly small. Once each piece is small, their sum is small,
    and that establishes the product rule.
  }
\end{frame}
...
\begin{frame}{Proof of 3.3(d): Reciprocal --- Bounding Away from 0}
  \begin{proofblock}{Step 1 --- Keep $|s_n|$ away from $0$}
    ...
    \[
      |s_n| = |s + s_n - s| \ge |s| - |s_n - s| > \tfrac{1}{2}|s| \quad (n \geq m).
    \]
  \end{proofblock}
...
\begin{frame}{Proof of 3.3(d): Reciprocal --- The Estimate}
  ...
  \note{
    Where does the strange tolerance one-half absolute value of "s"
    squared times epsilon come from? It is the result of working
    backward from the target. After putting the two reciprocals over
    a common denominator, the difference equals "s" sub "n" minus
    "s" divided by the product "s" sub "n" times "s". Step 1 told us
    the absolute value of "s" sub "n" is at least one-half the
    absolute value of "s", so the denominator's absolute value is at
    least one-half the absolute value of "s" squared. The whole
    quantity is therefore at most 2 over absolute value of "s"
    squared, times the absolute value of "s" sub "n" minus "s". To
    pin this final bound below epsilon, we need the numerator below
    one-half absolute value of "s" squared times epsilon, and that
    is exactly the tolerance we picked when applying convergence.
    The conclusion is that the reciprocal of "s" sub "n" converges
    to the reciprocal of "s".
  }
\end{frame}
...
% =============================================================================
\section{Convergence in $\mathbb{R}^k$}
% =============================================================================
...
\begin{frame}{Theorem 3.4: Componentwise Convergence}
  ...
  \vspace{0.3em}
  \begin{center}
    \hl{Vector convergence $\iff$ convergence in \alert{every} coordinate.}
  \end{center}
...
\begin{frame}{Theorem 3.4: Algebraic Operations in $\mathbb{R}^k$}
  ...
  \vspace{0.3em}
  \begin{center}
    \hl{Limits commute with sum, dot product, and scalar multiplication.}
  \end{center}
...
\begin{frame}{Proof of 3.4(a): $\Leftarrow$ Direction}
  ...
  \note{
    ...
    the matching coordinate of "x". The natural strategy is to give
    each coordinate its own small budget, so that the combined error
    stays under epsilon. Where does the funny choice epsilon over
    square root of "k" come from? Work backward. The squared norm of
    the difference vector is a sum of "k" squared coordinate
    deviations. If we want the sum to be at most epsilon squared,
    each squared deviation should be at most epsilon squared over
    "k". Taking square roots, each coordinate deviation should be at
    most epsilon over square root of "k". That is exactly the
    tolerance we apply to each coordinate sequence. Each gives its
    own threshold; we take the maximum to get a single index "N"
    that works for all "k" coordinates simultaneously. Beyond that
    "N", the Euclidean norm of the difference vector is at most
    epsilon, which is convergence of the vector sequence.
  }
\end{frame}
